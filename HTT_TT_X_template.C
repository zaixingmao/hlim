...
void 
HTT_TT_X(bool scaled=false, bool log=true, float min=0.1, float max=-1., string inputfile="root/$HISTFILE", const char* directory="tauTau_$CATEGORY")
...
#else
  if(std::string(inputfile).find("8TeV")!=std::string::npos){dataset = "CMS, 19 fb^{-1} at 8 TeV";}
#endif
...
////TH1F* EWK3   = refill((TH1F*)input->Get(TString::Format("%s/ZL"      , directory)), "ZL" ); InitHist(EWK3 , "", "", TColor::GetColor(222,90,106), 1001);
  ...
  if (scaled) {
    InitHist(data, data->GetXaxis()->GetTitle(), Form("events / %g", data->GetBinWidth(1))); InitData(data);
  }
  else {
    InitHist(data, data->GetXaxis()->GetTitle(), "events / bin"); InitData(data);
  }

  gErrorIgnoreLevel = 2000;
  TH1F* ref=(TH1F*)ttbar->Clone("ref");
//  ref->Add(EWK1 );
//  ref->Add(EWK2 );
////ref->Add(EWK3 );
  ref->Add(EWK );
...
//    ggH->Add(qqH);
  ...
  */
  ...
  data->SetMinimum(min);
  data->SetMaximum(max>0 ? max : std::max(std::max(maximum(data, log),
						   maximum(ttbar, log)),
					  std::max(maximum(ggH, log), std::max(maximum(VH, log), maximum(qqH, log)))
					  ));
  data->Draw("e");
...
*/  
  ...
  if(SIGNAL_SCALE!=1){
    ...
    //ggH->Print("all");
    char decay[1000] = "#rightarrowhh#rightarrow#tau#taubb";
    leg->AddEntry(ggH  , Form("H(260 GeV)%s %s", decay, ggH->GetZaxis()->GetTitle()), "L" );
    leg->AddEntry(qqH  , Form("H(300 GeV)%s %s", decay, qqH->GetZaxis()->GetTitle()), "L" );
    leg->AddEntry(VH   , Form("H(350 GeV)%s %s", decay, VH->GetZaxis()->GetTitle()), "L" );
  }
#endif
#ifdef ASIMOV
  leg->AddEntry(data , "sum(bkg) + H(125)"              , "LP");
#else
  leg->AddEntry(data , Form("%s", data->GetZaxis()->GetTitle()), "LP");
#endif
  //  leg->AddEntry(Ztt  , "Z#rightarrow#tau#tau"           , "F" );
  leg->AddEntry(ttbar, "t#bar{t}"                       , "F" );
  //  leg->AddEntry(EWK  , "Electroweak"                    , "F" );
  leg->AddEntry(Fakes, "QCD"                            , "F" );
  leg->AddEntry(EWK, "VV"                            , "F" );
  //  $ERROR_LEGEND
  leg->Draw();
  ...
  */
  ...
  TH1F* model = (TH1F*)ttbar ->Clone("model");
